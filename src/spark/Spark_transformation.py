...
from config.Spark_config import Set_SparkSession, set_spark_config
...
def run_etl_process():
    # 1. KHỞI TẠO
    # Giả định bạn đã có hàm test_spark() và set_spark_config()
    spark_wrapper = test_spark()
    spark = spark_wrapper.spark
    mysql_config = set_spark_config()
    ...
    # Hàm hỗ trợ kiểm tra field tồn tại
    ...
    if has_nested_field(df, "payload.issue"):
        df_issues = df.select(
            ...
            # Phải có .alias("locked") nếu không tên cột sẽ bị sai
        ...
        # Tạo khung rỗng đúng schema nếu không có issue
    ...
    # ====================================================
    # GHI DỮ LIỆU (Thứ tự: Cha trước -> Con sau)
    # ====================================================

    # Khởi tạo writer
    writer = SparkWriteToMysql(spark, mysql_config)
